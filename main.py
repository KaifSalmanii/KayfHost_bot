...
STD_LIBS = {"os","sys","time","json","re","math","random","asyncio","threading","urllib","datetime","traceback","logging","sqlite3","base64","hashlib","typing"}
LIB_MAP = {"telebot": "pyTelegramBotAPI", "bs4": "beautifulsoup4", "cv2": "opencv-python", "pyrogram": "pyrogram", "telethon": "Telethon", "requests": "requests"}
...
def get_main_menu():
    # 🔴 FIX: is_persistent=True lagaya taaki buttons gayab na hon
...
def get_cancel_menu():
    return ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="❌ Cancel")]], resize_keyboard=True, is_persistent=True)
...
async def sys_status_menu(m: types.Message, state: FSMContext):
    ...
    await m.reply(f"📊 **Cloud Status**\n🟢 Server: Online\n🤖 Bots Live: {bots}\n⚡ Ping: Ultra-Fast (Anti-Sleep)", reply_markup=get_main_menu())
...
async def donate_menu(m: types.Message, state: FSMContext):
    ...
    await m.reply_photo(photo=qr, caption=f"💰 UPI: `{UPI_ID}`", reply_markup=get_main_menu())
    asyncio.create_task(delete_after(m, 1))
...
async def list_projects(m: types.Message, state: FSMContext):
    ...
    if uid not in db["projects"] or not db["projects"][uid]: return await m.reply("❌ No active projects.", reply_markup=get_main_menu())
...
async def deploy_to_cloud(m, pname, uid, repo_id, bot_user="", is_new=False):
    ...
    try:
        ...
        # 🔴 FIX: main.py ke andar se flask code nikal diya, ab error conflict nahi hoga!
